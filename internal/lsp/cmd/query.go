...
const (
	// These are all the "emulation" strings that can be passed to the emulate
	// flag causing query to adjust its output to match the binary being emulated

	// emulateGuru tells query to emulate the output of the guru tool
	emulateGuru = "guru"
)
...
type query struct {
	...
	Emulate string `flag:"emulate" help:"compatability mode, causes the lsp to emulate another tool.\nvalues depend on the operation being performed"`
